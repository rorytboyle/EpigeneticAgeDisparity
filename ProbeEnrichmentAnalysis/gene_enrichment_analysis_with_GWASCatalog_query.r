...
# Should I filter to overlap = 2 OR just plot top 20?
...
sig_genes_plot <- sig_genes %>%
  ...
  filter(overlap > 1) %>%
  # OR filter 2) select the top 20 strongest signals
  # slice_head(n = 20) %>%
...
cat(sprintf("Genes with >1 CpG overlap (Top 20 plotted): %d\n", nrow(sig_genes_plot)))
...
print(combined_genes) # is filtering to overlap = 2 ok?
...
final_results <- all_results_with_variants %>%
  ...
  filter(!trait %in% c("LGALS3BP protein levels", # sneaks in because of search for ALS
                       "Cognitive empathy")) %>% # sneaks in because of search for cognitive
